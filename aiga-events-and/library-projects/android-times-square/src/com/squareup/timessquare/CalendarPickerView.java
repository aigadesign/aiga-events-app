...
public class CalendarPickerView extends ListView {
	...
	private SparseIntArray eventDates;
	...
	public void setEventDates(SparseIntArray eventDates) {
		this.eventDates = eventDates;
	}

	private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
		boolean isSameDateAsPrevious = false;
		...
		case SINGLE:
			// AIGA Modification:
			// We want to revert the calendar to a de-selected state when the
			// user selects the same date twice.
			if (selectedCals.size() == 1) {
				if (newlySelectedCal.getTimeInMillis() == selectedCals.get(0).getTimeInMillis()) {
					isSameDateAsPrevious = true;
				}
			}
			clearOldSelections();
			break;
	...
	private class MonthAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			monthView.init(months.get(position), cells.get(position), eventDates);
			return monthView;
		}
	}
	...
	List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
		...
		while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
			...
			for (int c = 0; c < 7; c++) {
				...
				weekCells.add(new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday, value,
						rangeState));
				cal.add(DATE, 1);
			}
		}
